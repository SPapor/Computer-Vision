...
while True:
    ...
    if best_bbox is not None:
        ...
        if not tracking:
            # Перевірка що bbox коректний
            if w > 5 and h > 5 and 0 <= x < W and 0 <= y < H:
                roi = frame[y:y+h, x:x+w]

                # Перевірка, що патч не пустий і не чорний
                if roi.size > 0 and roi.mean() > 5:
    ...
    if tracking:
        ...
        # --- якщо об’єкт втрачено — перезапустити
        if lost_frames > 30:
            tracking = False
            tracker = None

    # --- Якщо нікого не видно ---
    if best_bbox is None and not tracking:
        cv2.putText(frame, "Waiting...", (10, 30),
